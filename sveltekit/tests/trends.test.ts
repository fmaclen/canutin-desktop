...
test.describe('Trends', () => {
	...
	test.skip('Balance type performances are calculated correctly', async ({ page, baseURL }) => {
		...
		await expect(
			page.locator('.table__sortable--active', { hasText: '1 month' })
		).not.toBeVisible();
		const oneMonthInvestments = page.locator(
			'.table__tr:nth-child(2) .table__td:nth-child(3) .tableValue',
			{ hasText: '+8.88%' }
		);
		...
		await expect(
			page.locator('.table__sortable--active', { hasText: '6 months' })
		).not.toBeVisible();
		const sixMonthsOtherAssets = page.locator(
			'.table__tr:nth-child(3) .table__td:nth-child(4) .tableValue',
			{ hasText: '-3.2%' }
		);
		...
		await expect(
			page.locator('.table__sortable--active', { hasText: 'Year to date' })
		).not.toBeVisible();
		// const debtYearToDate = page.locator('.table__tr:nth-child(5) .table__td:nth-child(5) .tableValue', { hasText: "-39%" })
		const debtYearToDate = page.locator(
			'.table__tr:nth-child(4) .table__td:nth-child(5) .tableValue',
			{ hasText: '-24%' }
		);
		...
		const cashOneYear = page.locator(
			'.table__tr:nth-child(5) .table__td:nth-child(6) .tableValue',
			{ hasText: '+100%' }
		);
		...
		await expect(
			page.locator('.table__sortable--active', { hasText: '5 years' })
		).not.toBeVisible();
		await expect(
			page.locator('.table__tr:nth-child(1) .table__td:nth-child(7)', { hasText: '~' })
		).toBeVisible();

		await expect(page.locator('.table__sortable--active', { hasText: 'Max' })).not.toBeVisible();
		const maxNetWorth = page.locator(
			'.table__tr:nth-child(1) .table__td:nth-child(8) .tableValue--positive',
			{ hasText: '+1,382%' }
		);
		...
		const maxDebt = page.locator(
			'.table__tr:nth-child(4) .table__td:nth-child(8) .tableValue--negative',
			{ hasText: '+3,626%' }
		);
		...
		await expect(
			page.locator('.table__tr:nth-child(3) .tableValue--positive', { hasText: '0%' })
		).not.toBeVisible();
		await expect(
			page.locator('.table__tr:nth-child(3) .tableValue--negative', { hasText: '0%' })
		).not.toBeVisible();
		await expect(
			page.locator('.table__tr:nth-child(1) .table__td:nth-child(9)', { hasText: '100%' })
		).toBeVisible();
		await expect(
			page.locator('.table__tr:nth-child(5) .table__td:nth-child(9)', { hasText: '5.76%' })
		).toBeVisible();

		await page.locator('button', { hasText: 'Allocation' }).click();
		await expect(
			page.locator('.table__tr:nth-child(1) .table__td:nth-child(9)', { hasText: '5.76%' })
		).toBeVisible();
